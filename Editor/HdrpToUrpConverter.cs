...
namespace RenderPipelineConverter {
    public class HdrpToUrpConverter : EditorWindow {
        private class ShaderProperties {
            ...
            // Emission
            public static readonly CrossShaderProperty EmissionMap = new(hdrp: "_EmissiveColorMap", urp: "_EmissionMap",
                CrossShaderProperty.Type.Texture);

            public static readonly CrossShaderProperty EmissionColor = new(hdrp: "_EmissiveColor", urp: "_EmissionColor",
                CrossShaderProperty.Type.Color);
            ...
            // Masks
            public static readonly HdrpMaskRemapper HDRP_MainMask = new("_MaskMap", "_MetallicGlossMap", "_OcclusionMap", DetailMap.urp,
                "_SpecGlossMap");
        ...
        private static Material? ConvertMaterial(Material baseMat, FileInfo baseMatPath, bool isUnlit) {
            ...
            for (int i = 0; i < miscPropertyCount; i++) {
                ...
                if (EditorUtility.DisplayCancelableProgressBar("Converting material..",
                        $"Transferring over misc property '{propertyName}' on material '{baseMat.name}'", (float)i / miscPropertyCount)) {
            ...
            // NOTE: The "Property () already exists with a different type: 0" errors mean the type of the property isn't a texture
            {
                ...
                for (int i = 0; i < additional.Length; i++) {
                    ...
                    if (EditorUtility.DisplayCancelableProgressBar("Converting material..",
                            $"Transferring HDRP property '{property.hdrp}' to URP property '{property.urp}' on material '{baseMat.name}'",
                            (float)i / additional.Length)) {
                        cancelConversion = true;
                    }
                }
                ...
                if (EditorUtility.DisplayCancelableProgressBar("Converting material..",
                        $"Unpacking main HDRP mask textures for material '{baseMat.name}'", 0.8f)) {
                    cancelConversion = true;
                }
        ...
        private static Material UnpackMaskMap(Material baseMat, FileInfo baseMatPath, HdrpMaskRemapper mask, Material mat) {
            if (ReadableEditorTexture.Fetch(baseMat, baseMatPath, mask.hdrpMask, out string? maskTexturePath) is not { } maskMap ||
            ...
            try {
                ...
                Logging.LogError(
                    $"Call to GetPixels failed on mask map \"{maskMap.texture.name}\".\nDoes the HDRP mask texture field exist on its shader?\n\n{exception}");
                return mat;
            }
        ...
        private static Texture2D? createIndividualMaskTextureAsset(string maskTexturePath, int width, int height, Color[] pixels, string name,
            Color defaultColor) {
        ...
        public static ReadableEditorTexture? Fetch(Material mat, FileInfo matPath, string propertyId, out string? texturePath) {
            ...
            if (mat.HasTexture(propertyId) && mat.GetTexture(propertyId) is { } tex1) {
                ...
                if (assetImporter is not TextureImporter porter) {
                    HdrpToUrpConverter.Logging.LogError(
                        $"Failed cast to {nameof(TextureImporter)} for texture \"{propertyId}\" (\"{matPath.FullName}\")");
                    return null;
                }
